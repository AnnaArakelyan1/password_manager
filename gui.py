...
class PasswordManagerApp:
    ...
    def main_screen(self):
        ...
        self.reset_master_btn = tk.Button(self.root, text="Reset Master Password", command=self.reset_master_password)
        self.reset_master_btn.pack(pady=5)

        self.reset_service_pw_btn = tk.Button(self.root, text="Reset Selected Password", command=self.reset_service_password)
        self.reset_service_pw_btn.pack(pady=5)
    ...
    def reset_master_password(self):
     ...
     current_pw = simpledialog.askstring("Verify Password", "Enter current master password:", show='*')
     if not current_pw or not verify_master_password(current_pw):
        messagebox.showerror("Error", "Current master password is incorrect.")
        return


     new_pw = simpledialog.askstring("New Master Password", "Enter new master password:", show='*')
     if not new_pw or not check_password_strength(new_pw):
        messagebox.showerror("Weak Password",
                             "Master password must be at least 8 characters and include uppercase, lowercase, digit, and special character.")
        return

     confirm_pw = simpledialog.askstring("Confirm Password", "Confirm new master password:", show='*')
     if new_pw != confirm_pw:
        messagebox.showerror("Error", "Passwords do not match.")
        return

     
     old_key = derive_key(current_pw)
     new_key = derive_key(new_pw)

  
     entries = self.storage.get_all_passwords()
     for entry in entries:
        entry_id, service, username, encrypted_pw = entry
        try:
            decrypted_pw = decrypt(encrypted_pw, old_key)
            new_encrypted_pw = encrypt(decrypted_pw, new_key)

            # Update database with new encrypted password
            c = self.storage.conn.cursor()
            c.execute("UPDATE passwords SET password = ? WHERE id = ?", (new_encrypted_pw, entry_id))
            self.storage.conn.commit()
        except Exception as e:
            messagebox.showerror("Error", f"Failed to re-encrypt password for {service}: {str(e)}")
            return

 
     set_master_password(new_pw)

     messagebox.showinfo("Success", "Master password has been reset. Please log in again.")
     self.logout()
    ...
    def reset_service_password(self):
     ...
     selection = self.password_listbox.curselection()
     if not selection:
        messagebox.showerror("Error", "No password selected to reset")
        return
     index = selection[0]
     entry = self.entries[index]

     new_pw = simpledialog.askstring("Reset Service Password", f"Enter new password for {entry[1]}:", show='*')
     if not new_pw or not check_password_strength(new_pw):
        messagebox.showerror("Weak Password",
                             "Password must be at least 8 characters and include uppercase, lowercase, digit, and special character.")
        return

     new_encrypted = encrypt(new_pw, self.key)
     self.storage.update_password(entry[0], new_encrypted)
     messagebox.showinfo("Success", f"Password for {entry[1]} has been reset.")
     self.refresh_password_list()
